...
public class UserController {
    ...
    @GetMapping("/api/letty-link/v1/user/{username}")
...
//        }
    ...
    @GetMapping("/api/letty-link/v1/user/has-username")
    public Result<Boolean> hasUsername(@RequestParam("username") String username) {
        return Results.success(userService.hasUsername(username));
    }

    @PostMapping("/api/letty-link/v1/user")
    public Result<Void> register(@RequestBody UserRegisterReqDTO requestParam) {
        userService.register(requestParam);
        return Results.success();
    }

    @PutMapping("/api/letty-link/v1/user")
    ...
    @PostMapping("/api/letty-link/v1/user/login")
    ...
    // 检查一下用户是否登录
    @GetMapping("/api/letty-link/v1/user/check-login")
    public Result<Boolean> checkLogin(@RequestParam("username") String username, @RequestParam("token") String token) {
        return Results.success(userService.checkLogin(username, token));
    }

    @DeleteMapping("/api/letty-link/v1/user/logout")
